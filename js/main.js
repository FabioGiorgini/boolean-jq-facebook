...
function nascondiPost(i){
  $("#post_"+i+"").addClass("not-displayable");
}

//FUNZIONE PER loadPosts() => aggiunge il footer al post
function getFooter() {
  ...
  posts.forEach((post, i) => {
    $("#postsList").append(
      `<div class="card post" id="post_${i}">
          ...
          <div class="post-image"><img src="${post.image}" class="card-img-top"></div> ${getFooter()}`);
    });
}
...
$(document).ready(()=>{
  //1° PUNTO
  //Inserire nome utente e immagine da user
  $("#headerUserImg").attr("src", user.avatar);
  $("#headerUserName").html(user.name);
  $("#sidebarUserImg").attr("src", user.avatar);
  $("#sidebarUserName").html(user.name + " " + user.surname);
